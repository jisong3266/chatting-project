...
int key_input(char move, string& str, char ch, int i) {
    while (move != ENTER) {
        ...
        if (move == UP || move == DOWN || move == LEFT || move == RIGHT) {
            if (move != 'P' && move != 'H' && move != 'K' && move != 'M') {
                return -1;
            }
        }// 방향키 입력시
...
string check_badwords(string msg) {
    ...
    string replacement;
    int idx = 0;
    readFile.open("C:/Users/HP/Desktop/DB_project/chatting/badwords.txt");
    if (!readFile.is_open()) {
...
void game_rank() {
    ...
    try {
        ...
        stmt = con->createStatement();
        result = stmt->executeQuery("SELECT id, score FROM game_rank order by score desc limit 5");
        if (!result->next()) {
            ...
            do {
                ranking.push_back(result->getString(1) + "          :          " + result->getString(2));
                top5_count++;
            } while (result->next());
            goto_xy(15, 7);
            cout << ranking[0];
        ...
        // 사용자의 랭킹을 가져와 출력합니다
        pstmt = con->prepareStatement("SELECT id, score FROM game_rank WHERE id = ? order by score desc limit 1");
        pstmt->setString(1, id);
        result = pstmt->executeQuery();
        bool isUserInTop5 = false;
        int Top5 = 0; // 사용자가 상위 5에 포함되었는지 여부와 변수
        if (!result->next()) {
            goto_xy(25, 19);
            cout << "NO RANK";
        }
            ...
            goto_xy(25, 19);
            cout << myrank;
            // 사용자의 랭킹과 상위 5 랭킹을 비교하여 동일한 경우 표시
            for (int i = 0; i < 4; i++) {
                if (myrank == ranking[i]) {
                    isUserInTop5 = true;
                    Top5 = i + 1;
...
void game_room() {
    ...
    string game_id = id + "game";
    send(client_sock, game_id.c_str(), 8, 0);
    int score = 0;
