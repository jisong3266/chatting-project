...
SOCKET_INFO getSocketInfo(string username);
...
void if_badwords(int ti, const char* msg, int diff) {
    char box[MAX_SIZE] = { };
    string text = msg;
    sck_list[ti].count++;
    cout << "욕설 감지!" << endl;
    ...
    if (diff == 0)
        ...
        send_adult_msg(text.c_str(), "");
    else
        send_private_msg(text.c_str(), "");
    if (sck_list[ti].count == 3) {
        sck_list[ti].count = 0;
        text = "[공지] " + nickname + "님이 경고누적으로 3분간 대화하실 수 없습니다!";
        cout << sck_list[ti].count << endl;
        if (diff == 0)
            send_msg(text.c_str(), "");
        else if (diff == 1)
            send_adult_msg(text.c_str(), "");
...
void recv_msg(string user) {
    char buf[MAX_SIZE] = { };
    SOCKET_INFO sck_info = getSocketInfo(user);
    while (1) {
        num = 0;
        //Sleep(100);
        int i = 0;
        ...
        if (recv(sck_info.sck, buf, MAX_SIZE, 0) > 0) { // 오류가 발생하지 않으면 recv는 수신된 바이트 수를 반환. 0보다 크다는 것은 메시지가 왔다는 것.
            ...
            if (msg.find("-") != string::npos) {
                if_badwords(sck_info.ti, msg.c_str(), 0);
            }
            for (int i = 0; i < sck_list.size(); i++) {
                if (id == sck_list[i].user && first == "/w") {
                    ...
                    if (p_msg.find("-") != string::npos) {
                        if_badwords(sck_info.ti, msg.c_str(), -1);
...
SOCKET_INFO getSocketInfo(string user) {
