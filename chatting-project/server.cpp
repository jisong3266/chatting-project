...
void if_badwords(SOCKET_INFO &sck_info, const char* msg) {
    t = 180;
    char box[MAX_SIZE] = { };
    string text = msg;
    sck_info.count++;
    cout << "욕설 감지!" << endl;
    text = "[경고] " + nickname + "님이 욕설을 사용하셨습니다! 누적 경고 횟수 : " + std::to_string(sck_info.count);
    send_msg(text.c_str(), "");
    if (sck_info.count == 3) {
        sck_info.count = 0;
        ...
        while (t > 0) {
            int min = t / 60;
            int sec = t % 60;
            int bytes_received = recv(sck_info.sck, box, MAX_SIZE, 0);
            if (bytes_received > 0) { // 오류가 발생하지 않으면 recv는 수신된 바이트 수를 반환. 0보다 크다는 것은 메시지가 왔다는 것.
                ...
                send_private_msg(text.c_str(), sck_info.user);
            }
        }
        th3.join();
        num = 1;
